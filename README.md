<div align="center">

<!-- HEADER BANNER -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=1B1B3A&height=200&section=header&text=ALSYA%20KANAYA&fontSize=72&fontColor=FFC300&fontAlignY=38&desc=Full%20Stack%20Developer%20%2F%2F%20Builder%20of%20Things&descAlignY=60&descColor=F5F5F5&animation=twinkling&fontAlign=50" />

</div>

---

<div align="center">

```
┌─────────────────────────────────────────────────────────────────────┐
│  $ whoami                                                           │
│                                                                     │
│  > Full Stack Developer — I build products end-to-end.              │
│  > Obsessed with clean architecture & fast feedback loops.          │
   > Based in Indonesia 🇮🇩 · Coffee-fueled · Shipped code today.       
└─────────────────────────────────────────────────────────────────────┘
```

</div>

---

## ⚡ CURRENTLY

<table>
<tr>
<td width="50%">

```yaml
status:
  working_on: "Side project that will change everything™"
  learning:   "System Design + Distributed Systems"
  open_to:    "Collabs, freelance, interesting ideas"
  mood:       "🔥 Building in public"
```

</td>
<td width="50%">

```yaml
quick_stats:
  repos:         48
  contributions: "1.2K+ commits"
  stars_earned:  230
  cups_of_coffee: "∞"
```

</td>
</tr>
</table>

---

## 🛠️ TECH STACK

<div align="center">

**[ LANGUAGES ]**

![TypeScript](https://img.shields.io/badge/TypeScript-1B1B3A?style=for-the-badge&logo=typescript&logoColor=FFC300)
![JavaScript](https://img.shields.io/badge/JavaScript-1B1B3A?style=for-the-badge&logo=javascript&logoColor=FFC300)
![Go](https://img.shields.io/badge/Go-1B1B3A?style=for-the-badge&logo=go&logoColor=FFC300)
![Python](https://img.shields.io/badge/Python-1B1B3A?style=for-the-badge&logo=python&logoColor=FFC300)
![Bash](https://img.shields.io/badge/Bash-1B1B3A?style=for-the-badge&logo=gnubash&logoColor=FFC300)

**[ FRONTEND ]**

![React](https://img.shields.io/badge/React-1B1B3A?style=for-the-badge&logo=react&logoColor=FFC300)
![Next.js](https://img.shields.io/badge/Next.js-1B1B3A?style=for-the-badge&logo=next.js&logoColor=F5F5F5)
![Tailwind](https://img.shields.io/badge/Tailwind-1B1B3A?style=for-the-badge&logo=tailwindcss&logoColor=FFC300)

**[ BACKEND & INFRA ]**

![Node.js](https://img.shields.io/badge/Node.js-1B1B3A?style=for-the-badge&logo=node.js&logoColor=FFC300)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-1B1B3A?style=for-the-badge&logo=postgresql&logoColor=FFC300)
![Redis](https://img.shields.io/badge/Redis-1B1B3A?style=for-the-badge&logo=redis&logoColor=FFC300)
![Docker](https://img.shields.io/badge/Docker-1B1B3A?style=for-the-badge&logo=docker&logoColor=FFC300)
![Linux](https://img.shields.io/badge/Linux-1B1B3A?style=for-the-badge&logo=linux&logoColor=FFC300)

</div>

---

## 📊 GITHUB STATS

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=alsyakd&show_icons=true&theme=tokyonight&hide_border=true&bg_color=1B1B3A&title_color=FFC300&icon_color=FFC300&text_color=F5F5F5&ring_color=FFC300" />
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=alsyakd&layout=compact&theme=tokyonight&hide_border=true&bg_color=1B1B3A&title_color=FFC300&text_color=F5F5F5" />

</div>

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=alsyakd&theme=dark&hide_border=true&background=1B1B3A&ring=FFC300&fire=FFC300&currStreakLabel=FFC300&sideLabels=F5F5F5&dates=777&currStreakNum=F5F5F5&sideNums=F5F5F5)](https://git.io/streak-stats)

</div>

---

## 🚀 PINNED PROJECTS

<div align="center">

| Project | What it does | Stack | Stars |
|--------|-------------|-------|-------|
| 🔧 **[project-alpha](https://github.com/alsyakd/project-alpha)** | REST API boilerplate — auth, rate limiting, ready for prod | `TypeScript` `Node` `Docker` | ⭐ 87 |
| 🖥️ **[dashboard-kit](https://github.com/alsyakd/dashboard-kit)** | Dark-first admin dashboard. Zero fluff, ships in 5 mins | `React` `Tailwind` | ⭐ 64 |
| 🗄️ **[sql-migrator](https://github.com/alsyakd/sql-migrator)** | Zero-downtime PostgreSQL migrations via CLI | `Go` `PostgreSQL` | ⭐ 41 |
| 📓 **[devlog](https://github.com/alsyakd/devlog)** | Markdown dev journal with tagging & CLI sync | `Node.js` | ⭐ 38 |

</div>

---

## 📈 CONTRIBUTION GRAPH

<div align="center">

[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=alsyakd&bg_color=1B1B3A&color=FFC300&line=FFC300&point=F5F5F5&area=true&hide_border=true)](https://github.com/ashutosh00710/github-readme-activity-graph)

</div>

---

## 🎯 THIS WEEK I...

<!--START_SECTION:waka-->
```text
TypeScript   █████████████░░░░░░░░   58.4%
Go           ████░░░░░░░░░░░░░░░░░   18.2%
SQL          ██░░░░░░░░░░░░░░░░░░░    9.1%
Bash         █░░░░░░░░░░░░░░░░░░░░    7.7%
Markdown     █░░░░░░░░░░░░░░░░░░░░    6.6%
```
<!--END_SECTION:waka-->

> 💡 *Powered by [WakaTime](https://wakatime.com). Auto-updates weekly.*

---

## 💬 RANDOM DEV QUOTE

<div align="center">

![Quote](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=dark&bg_color=1B1B3A&border_color=FFC300&color=F5F5F5)

</div>

---

## 🔗 FIND ME

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-1B1B3A?style=for-the-badge&logo=linkedin&logoColor=FFC300)](https://linkedin.com/in/alsya-kanaya-dzikra-akd07)
[![Twitter/X](https://img.shields.io/badge/X_(Twitter)-1B1B3A?style=for-the-badge&logo=x&logoColor=FFC300)](https://x.com/akd1027)
[![Portfolio](https://img.shields.io/badge/Portfolio-1B1B3A?style=for-the-badge&logo=firefox&logoColor=FFC300)](https://yoursite.dev)
[![Email](https://img.shields.io/badge/Email-1B1B3A?style=for-the-badge&logo=gmail&logoColor=FFC300)](mailto:alsyakanayadzikra@gmail.com)

</div>

---

<div align="center">

<!-- FOOTER WAVE -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=1B1B3A&height=120&section=footer&fontColor=FFC300" />

<sub>⚡ Profile views: <img src="https://komarev.com/ghpvc/?username=YOUR_USERNAME&color=FFC300&style=flat-square&label=views" /></sub>

</div>
